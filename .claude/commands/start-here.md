...
Canonical first-run onboarding for AI-OS. Use this command when someone has just installed AI-OS, copied the template, opened a fresh client workspace, or asks to set up the system, gather context, build brand context, choose skills, or understand what to do next.

The job is to walk the user through setup by Q/A, one answer at a time, until AI-OS has enough context to be useful. Do not turn this into a docs dump. Ask, listen, save the durable context, show what was built, then recommend the first useful task.

`/onboarding` is a compatibility alias for this command.

## Operating rules

- Ask one question at a time. Wait for the answer before asking the next.
- Skip any question the user already answered. Briefly say what you inferred so they can correct it.
- Do not ask more than four core business or taste questions before moving into source collection and durable file creation.
...
- Never rebuild existing `brand_context/` without the user's explicit request.
...
- Use real repo state. Scan files and scripts before explaining what exists.
...
- If `gh` is installed and authenticated, offer to create a private repo and wire remotes:
  `gh repo create my-ai-os --private --source=. --remote=origin`
- If `gh` is unavailable, give the manual path:
  `git remote rename origin upstream && git remote add origin <their-private-repo-url> && git push -u origin main`
...
Do not block onboarding if they choose to continue without backup. Record the deferred backup risk in the session notes.
...
Then stop. If the user explicitly asked to rerun onboarding, continue and ask before overwriting any existing brand context.

If selection is incomplete, still complete skill selection before finishing, even if brand context already exists.
...
- Skills are optional capabilities the user can keep, remove, or add later.
...
Before writing the brand files, read the relevant foundation skills:
