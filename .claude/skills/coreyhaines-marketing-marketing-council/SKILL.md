---
name: coreyhaines-marketing-marketing-council
description: "Convene a simulated board of legendary marketers (Godin, Ogilvy, Schwartz, Dunford, Sutherland, Hormozi, Sharp) who each weigh in through their real frameworks, surface where they disagree, then synthesize a recommendation. Not for executing the chosen direction, hand off to the relevant execution skill (coreyhaines-marketing-offers, coreyhaines-marketing-copywriting, coreyhaines-marketing-ads)."
when_to_use: 'Invoke when the request sounds like: ''marketing council'', ''board of advisors'', ''what would Seth Godin say'', ''channel Hormozi'', ''get multiple perspectives'', ''debate this'''
metadata:
  version: 1.0.0
---
...
| `context/learnings.md` | `## coreyhaines-marketing-marketing-council` | Past corrections and preferences for this skill |

## Before Starting

**Check for brand context first:**
Read the AI-OS brand context in `brand_context/` before asking questions: `positioning.md`, `icp.md`, and `voice-profile.md` (plus `samples.md` / `assets.md` when useful). If those files do not exist yet, ask the user, or offer to build them with `coreyhaines-marketing-product-marketing` or the AI-OS foundation skills `mkt-positioning`, `mkt-icp`, and `mkt-brand-voice`.
...
- **If a deep-research skill is installed** (e.g., `coreyhaines-skills-deep-research`): use it to find what the seated advisors have actually said or written about this topic class - books, essays, interviews, podcasts - plus current state of the debate.
- **If a video-analysis skill is installed** (e.g., `coreyhaines-skills-watch-video`): pull takes from specific talks/interviews the research surfaces.
- **If a recency skill is installed** (e.g., `str-trending-research`): check for recent takes when the topic is fast-moving.
...
Users can extend the bench ("add my own advisor"). Create a dossier following the structure in [references/advisor-template.md](references/advisor-template.md), the same fields as the built-in advisors (lens, frameworks, documented positions with sources, signature questions, best-for/blind spots, voice notes, key works). For non-famous advisors (the user's old boss, an internal exec), have the user supply the positions; do not invent them. Save to `projects/coreyhaines-marketing-marketing-council/advisors/<name>.md` so it persists and never collides with skill updates.
...
- **mkt-positioning** / **coreyhaines-marketing-product-marketing**: When Dunford's take wins - execute the positioning work
- **coreyhaines-marketing-offers** / **coreyhaines-marketing-pricing**: When Hormozi/Halbert direction wins - build the offer
- **coreyhaines-marketing-copywriting** / **coreyhaines-marketing-copy-editing**: When the council reviewed copy - execute revisions
- **coreyhaines-marketing-ads** / **coreyhaines-marketing-ad-creative**: When the debate was media or creative strategy
- **coreyhaines-marketing-content-strategy** / **coreyhaines-marketing-social**: When Handley/Vaynerchuk direction wins
- **mkt-brand-voice** / **coreyhaines-marketing-marketing-psychology**: For Sharp's availability work and Sutherland's behavioral mechanics
- **coreyhaines-marketing-ab-testing**: When the disagreement map says "test it" - Hopkins would insist
- **coreyhaines-skills-deep-research**: For the live research pass, when installed
