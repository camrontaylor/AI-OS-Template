...
Copy this structure to add an advisor to the bench. Save custom advisors to `projects/coreyhaines-marketing-marketing-council/advisors/<kebab-name>.md` (not inside the skill folder) so they survive skill updates.
...
**Seating a custom advisor:** mention them by name when convening ("seat my advisor Maria on this council"). The agent loads the file from `projects/coreyhaines-marketing-marketing-council/advisors/` and treats it like any bench dossier, including the grounding rules: no fabricated quotes, no invented endorsements.
