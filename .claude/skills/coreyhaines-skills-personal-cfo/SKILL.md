---
name: coreyhaines-skills-personal-cfo
description: "Models personal and household financial scenarios: house purchase plus rental income, renovation budgets, monthly cash flow, big-purchase decisions, and savings what-ifs, output as markdown comparison tables saved and indexed for revisit. (needs your own bank and finance figures)"
when_to_use: 'Invoke when the request sounds like: "house math", "rental forecast", "monthly cash flow", "should we buy this house", "compare these housing scenarios", "renovation budget"'
metadata:
  version: 0.2.0
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-personal-cfo` | Past corrections and preferences for this skill |
...
Output to `projects/coreyhaines-skills-personal-cfo/<scenario-slug>-<YYYY-MM-DD>/` (the AI-OS home for this skill's reusable artifacts; create the folder on first use):
...
Save the scenario folder so future revisits work. The archive index lives in `projects/coreyhaines-skills-personal-cfo/INDEX.md` (create the folder if missing). This is the AI-OS home for the skill's reusable artifacts, so it survives skill updates. Never write archives inside the skill's own folder, updates re-sync it from source and wipe anything saved there. **Migration:** if this skill's folder contains an old `references/scenarios-archive/` with user entries, move those files into `projects/coreyhaines-skills-personal-cfo/` first.

Append a one-liner to `projects/coreyhaines-skills-personal-cfo/INDEX.md` (create if missing):

```markdown
- 2026-06-26, [<property nickname>](projects/coreyhaines-skills-personal-cfo/<slug>-2026-06-26/), <type>, <headline takeaway>
```
...
- Offer:
  - *"Want to run `coreyhaines-skills-deep-research` on rental comps for the area to firm up the ADU and bedroom rent assumptions?"*
  - *"Want to formalize the buy/don't-buy decision through `coreyhaines-skills-decide` once the scenarios land?"*
  - *"Want to capture this analysis as a durable resource via `str-resources` (or as a note in `projects/coreyhaines-skills-personal-cfo/`)?"*
  - *"Want me to revisit in 30 days as more info comes in (loan rate locked, inspection done, etc.)?"*

## Composes with

- **`coreyhaines-skills-decide`** - once scenarios are modeled, the buy / don't / negotiate decision goes through it. Cash flow analysis is input, not the decision itself.
- **`coreyhaines-skills-deep-research`** - for inputs that need market data: rental comps, mortgage rate trends, neighborhood trajectory, school ratings, comparable sales.
- **`coreyhaines-skills-business-brainstorm`** - if the scenario tips into "this is a business" (e.g., short-term rental, multi-unit purchase, scaling to N properties), route there to pressure-test the business angle.
- **`str-resources`** - capture the analysis as a durable AI-OS resource (indexes it into the resource system and Notion) so it can be recalled later.
