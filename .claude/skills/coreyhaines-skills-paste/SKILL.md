---
name: coreyhaines-skills-paste
description: "Cleans and reformats content (usually terminal output) for pasting into Slack, Notion, Twitter/X, LinkedIn, email, or GitHub: strips ANSI codes, box-drawing chars, and prompt artifacts, applies destination formatting, reads and writes the clipboard, and warns on secrets. Not for writing new content."
when_to_use: 'Invoke when the request sounds like: "clean this for X", "format for slack/notion/twitter", "make this copy-pastable", "strip formatting", "render as markdown"'
metadata:
  version: 0.1.1
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-paste` | Past corrections and preferences for this skill |
...
- See `coreyhaines-skills-jab-hook` and `context/learnings.md` under `## coreyhaines-skills-jab-hook`
...
- `coreyhaines-skills-jab-hook` - clean output for pasting into Typefully drafts when the MCP path doesn't fit; also enforces the link-placement rule for X/LinkedIn destinations
- `coreyhaines-skills-social-fetch` - clean a fetched post before quoting it in a draft or newsletter
- `coreyhaines-skills-second-brain` - `paste` output can be captured cleanly into `raw/note-<slug>.md` for future compilation
- `coreyhaines-skills-watch-video` - transcript / summary output often flows through `paste` for platform-specific reformatting
...
- **Links go in first comments for social destinations** - not the body. Enforced when destination is `twitter` or `linkedin`. Documented in `context/learnings.md` under `## coreyhaines-skills-jab-hook`.
