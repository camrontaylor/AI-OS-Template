---
name: coreyhaines-skills-watch-video
description: "Transcribes and analyzes any video (YouTube, Loom, Vimeo, Riverside, Zoom, local MP4, X/IG, anything yt-dlp supports) at a chosen depth: transcript only, visual frame analysis, or multimodal, saving notes and optionally capturing a summary to a second brain. Not for short articles (str-resources)."
when_to_use: 'Invoke when the request sounds like: "watch this video", "transcribe this loom", "analyze this video", "summarize this recording", "key moments from this"'
metadata:
  version: 0.2.2
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-watch-video` | Past corrections and preferences for this skill |
...
- **X / IG / TikTok video**: URL, defers to `coreyhaines-skills-social-fetch` for metadata, uses yt-dlp for the file
...
```
projects/coreyhaines-skills-watch-video/<source>-<slug>-<date>/
```
...
- Path to workdir
- (Optional) Step 9 - offer to capture the summary as a durable resource
...
## Step 9 - Optional: capture the summary

After any mode completes, offer:

> *"Want to capture this as a durable resource? I'll route the summary, source URL, and transcript link through `str-resources` (which indexes it into the AI-OS resource system and Notion). If you keep an Obsidian vault, `coreyhaines-skills-second-brain` can file it to `raw/` as a `call-`/`meeting-`/`note-`/`resource-` note instead."*

Type prefix by source (when filing into a `coreyhaines-skills-second-brain` vault):
...
- If captured as a resource: that path too
...
| X / IG / TikTok | Defer to `coreyhaines-skills-social-fetch` for metadata, yt-dlp for file | No | Short-form |
| Local file | n/a | n/a | Drop a path |

## Composes with

- `coreyhaines-skills-social-fetch` - for X/IG/TikTok URL metadata (engagement, author, replies) before video processing
- `str-resources` (or `coreyhaines-skills-second-brain`) - capture the summary as a durable AI-OS resource, or file it to an Obsidian vault as a `call-`/`meeting-`/`note-`/`resource-` note
- `tool-youtube` - AI-OS's YouTube-native sibling for the transcript path on YouTube URLs
- `coreyhaines-skills-decide` - when a video contains a flagged decision, route to it for structured capture
- `coreyhaines-skills-pm` - action items flagged in summary can be triaged to project boards
- `coreyhaines-skills-slide-deck` - talk recordings to outline extraction to deck draft (loop)
- `coreyhaines-skills-jab-hook` - quotes + clip-worthy moments from podcast/talk videos feed BIP/promo posts
- **`coreyhaines-skills-skillify from-video`** - primary use case for `visual` mode on process recordings. The user records themselves doing a workflow (Loom/screen-share), this skill extracts transcript + key visual moments, then `coreyhaines-skills-skillify` synthesizes the workflow into a SKILL.md. "Record once, AI converts to skill."
