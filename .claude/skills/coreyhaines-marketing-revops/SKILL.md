---
name: coreyhaines-marketing-revops
description: "Design and optimize the systems that connect marketing, sales, and success into one revenue engine - lead scoring, lead routing, MQL/SQL definitions, pipeline stages, CRM automation, data hygiene, the marketing-to-sales handoff. Not for cold outreach (see coreyhaines-marketing-cold-email), drip campaigns (see coreyhaines-marketing-emails), or pricing (see coreyhaines-marketing-pricing)."
when_to_use: 'Invoke when the request sounds like: ''RevOps'', ''lead scoring'', ''lead routing'', ''MQL to SQL'', ''pipeline stages'', ''leads aren''t getting to sales'''
metadata:
  version: 2.0.0
---
...
| `context/learnings.md` | `## coreyhaines-marketing-revops` | Past corrections and preferences for this skill |

## Before Starting

**Check for product marketing context first:**
Read AI-OS brand context in `brand_context/` - `positioning.md`, `icp.md`, `voice-profile.md` (and `samples.md`/`assets.md` when useful) - before asking questions. Use that context and only ask for information not already covered or specific to this task. If those files do not exist yet, ask the user or offer to build them with `coreyhaines-marketing-product-marketing` or the AI-OS foundation skills `mkt-positioning` / `mkt-icp` / `mkt-brand-voice`.
...
For implementation, see the [tools registry](docs/connectors.md). Key RevOps tools:

| Tool | What It Does | Guide |
|------|-------------|-------|
| **HubSpot** | CRM, marketing automation, lead scoring, workflows | [hubspot.md](docs/connectors.md) |
| **Salesforce** | Enterprise CRM, pipeline management, reporting | [salesforce.md](docs/connectors.md) |
| **Calendly** | Meeting scheduling, round-robin routing | [calendly.md](docs/connectors.md) |
| **SavvyCal** | Scheduling with priority-based availability | [savvycal.md](docs/connectors.md) |
| **Clearbit** | Real-time lead enrichment and scoring | [clearbit.md](docs/connectors.md) |
| **Apollo** | Contact data, enrichment, and outbound sequences | [apollo.md](docs/connectors.md) |
| **ActiveCampaign** | Marketing automation for SMBs, lead scoring | [activecampaign.md](docs/connectors.md) |
| **Zapier** | Cross-tool automation and workflow glue | [zapier.md](docs/connectors.md) |
| **Introw** | Partner-sourced pipeline, commissions, deal registration, QBRs | [introw.md](docs/connectors.md) |
| **Crossbeam** | Partner account overlaps and co-sell identification | [crossbeam.md](docs/connectors.md) |
