...
### Suggested wiki pages that compose with other skills

Pages whose existence makes other skills sharper:

| Wiki page | Composes with | Why |
|---|---|---|
| **Decision Log** | `coreyhaines-skills-decide` | Narrative version of decisions made - the `coreyhaines-skills-decide` skill archives one-off decisions; the wiki page is the story of how decisions stacked over time on a topic |
| **Content Ideas** | `coreyhaines-skills-jab-hook` | Centralized hopper of hooks, frameworks, stories - drafted posts pull from here |
...
| **Workflow Docs** | `coreyhaines-skills-pm` | Documented operational patterns - what gets done, by whom, on what cadence |
...
- Research briefs (composes with `coreyhaines-skills-deep-research`)
- Sales scripts
- Client summaries
- Slide deck content (handed off to `coreyhaines-skills-slide-deck`)
...
Styling is optional. Set the `RENDER_CSS` env var to point at your own stylesheet (`coreyhaines-skills-read-book` uses the same convention); unset, pandoc uses its clean defaults. A system-font + max-width 720 + print-optimized sheet works well.
