---
name: coreyhaines-skills-second-brain
description: "Captures into, compiles, queries, lints, or connects a personal Second Brain markdown vault (Karpathy LLM Wiki schema: raw sources, AI-compiled interlinked wiki pages, generated outputs); modes for capture, compile, query, lint, connect, search. (needs your own Obsidian or markdown vault)"
when_to_use: 'Invoke when the request sounds like: "capture this", "save this to my brain", "compile the wiki", "query my wiki", "ask my brain", "find connections"'
metadata:
  version: 0.1.0
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-second-brain` | Past corrections and preferences for this skill |
...
Answer a question using ONLY the wiki/raw corpus. Different from `coreyhaines-skills-deep-research` (which goes external).
...
3. **Compose the answer**:
   ...
   - If the wiki doesn't contain the answer, say so and offer to run `coreyhaines-skills-deep-research` to expand
...
- `coreyhaines-skills-deep-research` - when `query` finds gaps in the wiki, route to it to expand from external sources. Its output can be captured back into `raw/` for future compilation.
- `coreyhaines-skills-paste` - capture content cleanly into `raw/` (especially for terminal/CLI captures).
- `coreyhaines-skills-business-brainstorm` - checks `Portfolio of Businesses` and `Entrepreneurship & Startups` wiki pages for relevant context before brainstorming.
- `coreyhaines-skills-decide` - pull from `Personal Philosophy` / `Productivity & Systems` wiki for principles when scoring Q34 ("what principles are we bending"). A `Decision Log` wiki page accumulates the narrative form of decisions over time (the `coreyhaines-skills-decide` archive is the structured form; the wiki page is the story).
- `coreyhaines-skills-jab-hook` - a `Content Ideas` wiki page hoppers hooks, frameworks, and stories. Its drafts pull candidates from there.
- `coreyhaines-skills-slide-deck` - content drafted in `outputs/` becomes deck source; speaker notes can reference relevant wiki pages.
- `coreyhaines-skills-pm` - Projects/ folder in the vault is off-limits to second-brain; pm owns it. But a `Workflow Docs` wiki page captures operational patterns that show up across multiple projects.
