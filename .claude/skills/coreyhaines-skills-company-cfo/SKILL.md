---
name: coreyhaines-skills-company-cfo
description: "Run a monthly company CFO workflow - pull bank, payment, payroll, and expense data, categorize and reconcile, compute end-of-month cash by transaction sum, update a scenario projector, and write the leadership snapshot report. Modes: monthly, weekly, scenario, pickup (needs your own data setup). Not for household finances (see coreyhaines-skills-personal-cfo)."
when_to_use: 'Invoke when the request sounds like: "company CFO", "monthly cash report", "CFO snapshot", "cash projection", "runway forecast"'
metadata:
  version: 0.1.1
---
...
Anonymized team-scope sibling to `coreyhaines-skills-personal-cfo` (households). Same discipline (transaction-sum EOM, categorization traps, scenario modeling) applied to company books.
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-company-cfo` | Past corrections and preferences for this skill |

## Step 0 - Load company config + prior run

Before starting work, read these in order:

1. **`${COMPANY_CFO_ROOT:-$HOME/company-cfo}/CLAUDE.md`** (needs your own finance repo) - your company's specific methodology, data source map, categorization rules, distribution mechanics. Set `COMPANY_CFO_ROOT` to point at the private repo where your bank, payroll, and payment data live. **This is the source of truth for HOW your company computes things.** Don't invent your own methodology.
...
4. **Any relevant AI-OS memory notes** - running context: known anomalies, leadership constraints, current churn state. Check `context/MEMORY.md`, today's `context/memory/` log, and `projects/coreyhaines-skills-company-cfo/` (AI-OS keeps native Claude memory off, so nothing lives in `~/.claude/memory/`).
...
**If a source isn't wired yet:** use `coreyhaines-skills-toolify <source>` to wire it up before running the CFO workflow. First-time integration is a one-time cost.
...
Update `projects/coreyhaines-skills-company-cfo/company_cfo_<company-slug>.md` (the skill's archive folder) if any of:
...
Don't bloat the note. Replace stale facts; don't append indefinitely. For a durable fact leadership will lean on later, route it through `meta-memory-write` into `context/MEMORY.md` instead.
...
Pair with `coreyhaines-skills-loopify` (or AI-OS's own `ops-cron`) to schedule the weekly run (typically Monday 9am).
...
If the scenario decision is material (new hire, distribution change, big expense), also run `coreyhaines-skills-decide` to formalize.
...
- **`coreyhaines-skills-personal-cfo`** - sibling. Personal-cfo handles household finances (house math, monthly cash flow, big purchases). Same discipline (transaction-sum method, scenario modeling) applied to different scope.
- **`coreyhaines-skills-company-brain`** - the CFO reports get stored + wiki-indexed there. `outputs/` in the company brain accumulates monthly reports; wiki pages compile trends across months.
- **`coreyhaines-skills-toolify`** - wire company-specific data sources (Mercury API, Stripe, Plane, Ramp, or equivalents). First-run integration setup.
- **`coreyhaines-skills-loopify`** (or AI-OS's own `ops-cron`) - schedule the monthly run (1st of month) + weekly cash pulse (Monday 9am).
- **`coreyhaines-skills-decide`** - for material decisions surfaced by the report (distribution changes, hiring, cash floor breach response). Formalize with the 37signals framework.
- **`coreyhaines-skills-deep-research`** - for benchmark questions ("what's a typical SaaS marketing budget as % of ARR?") that inform scenario inputs.
...
- **Personal finance** - use `coreyhaines-skills-personal-cfo` instead.
