...
The file becomes the source of truth for HOW your company computes its financials. The `coreyhaines-skills-company-cfo` skill reads this first before every run.
...
The `coreyhaines-skills-company-cfo` skill's Phase 7 seeds this automatically if missing, but doing it manually at repo setup is safer.
...
Running context lives at `projects/coreyhaines-skills-company-cfo/company_cfo_<slug>.md` (AI-OS keeps native Claude memory off). Update if:
