...
Each technique: when it fires (by wall type), the moves, and a worked example. Run 3 to 4 per session (T1 always runs). Generate 2 to 3 angles per technique; do not evaluate while generating.
...
| Resource | T1 | T7 | T8 |
| Physics/math | T1 (confirm it is real) | honest exit: reroute the goal | T4 (the rerouted goal often hides one altitude up) |
...
3. Mark each: **verified fact** (you checked, recently, yourself) vs **inherited belief** (docs said, someone said, it has always been that way, you assumed).
4. For each belief: what would it cost to test it right now? Test the cheap ones immediately.

**Example:** "The OG-image renderer cannot draw the cursor, it renders a box glyph." Assumptions: (a) the cursor must be a *text character*, belief; (b) the default font is the only font, belief, the renderer accepts custom fonts; (c) the design needs a cursor at all, belief. Breaking (a) solves it in minutes: draw the cursor as an orange rectangle `<div>`, not a glyph. (Breaking (b) was the fix for the checkmark next to it, load a font that has one.)

## T2, inversion

**Fires on:** framing walls; any wall where forward motion has stalled.

**Moves:** Ask "how would I *guarantee* this stays blocked or fails completely?" List those answers earnestly, then negate each one. The negations are angles. (Jacobi via Munger: "Invert, always invert.")

**Example:** "Cannot get replies to cold outreach." Guarantee zero replies: generic opener, ask for 30 minutes, no reason to reply this week, sent to the wrong person. Negations: hyper-specific opener referencing their work this week; ask for nothing (give something); find the person who *owns the pain*, not the title.

## T3, first principles

**Fires on:** assumption walls, tool walls with deep inherited design.

**Moves:** Strip the problem to what physics, math, or economics actually require. Ignore how it is currently done, that is an artifact of someone else's constraints. Rebuild the minimum path from requirements up. Ask: "if nobody had ever solved this before, and I had today's tools, what would I do?"

**Example:** "Shipping costs make this product unviable." First principles: what does the customer actually need to *receive*? If the value is information plus a few grams of material, ship the file and license local production, the freight was an artifact of a pre-digital design.
...
**Moves:** Assume it shipped. Write the one-paragraph changelog entry or press release dated 3 months out. Now ask: what MUST have been true for that to happen? Chain backwards from the solved state to today. The first link backwards from "solved" is often visible even when the first step forward is not.

**Example:** "No idea how to get our first 100 users." Changelog from the future: "100 users, 60 from the launch partner's newsletter." Must-have-been-true: a launch partner with an audience who wins by sharing this. That is the actual task, find *that partner*, not "do marketing."
...
**Example:** "Users abandon our 9-step onboarding." Structure: mandatory sequence with dropout. Who else: airport security (move the wait into parallel or idle time), video games (tutorial disguised as play). Angles: collect steps 4 to 9 lazily *during* first use; make step 1 deliver the aha before asking for anything.
...
1. **Remove it:** "If money, time, or headcount were free, what would I do?" Then ask what a 5% version of that answer costs.
2. **Tighten it 10x:** "If I had one hour, $100, or just myself, what would I do?" Brutal constraint forces the essential move to the surface.
Both directions expose which constraints are real and which are assumed comfort.

**Example:** "Cannot afford a rebrand ($40k agency quote)." Free-money version: full agency process, its 5% core is one senior designer's taste applied to logo, type, and 3 templates. 10x-tighter version ($4k): hire that one senior designer for a week. The agency was packaging, not the value.
...
**Moves:** (de Bono) State a deliberately wrong or absurd version of the situation ("Po: customers pay us to see ads," "Po: the app has no interface"), then harvest the *movement*: what direction does the absurdity point? Alternative: random entry, pick an unrelated noun (open a book, point), force-connect it to the problem, harvest.
...
**Moves:** (Eberle's mnemonic, built on Osborn's brainstorming checklist.) Against the component that "cannot": **S**ubstitute (different primitive, library, channel), **C**ombine (merge with an adjacent step), **A**dapt (borrow a mechanism that works elsewhere in the system), **M**agnify/minify (what if this step were 10x bigger or disappeared?), **P**ut to other use (can an existing feature be abused into doing this?), **E**liminate (does this step need to exist?), **R**everse (flip the order, direction, or actor).

**Example:** "The metadata route cannot re-export another route's component" (Next.js twitter-image, 2026-07). Substitute: literal config exports plus delegated default import, same dedupe, analyzable statically. Eliminate was also on the table: delete twitter-image entirely and let og:image serve both.
...
3. **What is their incentive?** What does yes cost them (risk, effort, precedent)? Can you remove that cost?
...
5. **Guardrail:** if they understood the ask and meant the no, that no is real. The move is finding a different door they would *happily* open, never pushing the closed one.
...
- **Premature feasibility**: "that will not work because..." during generation kills the wild angle that would have seeded the viable one. Triage is Step 5, not Step 4.
