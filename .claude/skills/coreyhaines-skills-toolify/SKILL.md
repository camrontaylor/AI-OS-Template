---
name: coreyhaines-skills-toolify
description: "Integrates an external tool, API, MCP server, or service into a Next.js or Rails project: walks auth, config, env vars, client wrapper code, example usage, .mcp.json wiring, and an optional smoke-test, then scaffolds the plumbing. Not for new skills (coreyhaines-skills-skillify) or loops (coreyhaines-skills-loopify)."
when_to_use: 'Invoke when the request sounds like: "integrate X", "add X to this project", "wire up X", "set up the X integration", "connect X", "add MCP for X"'
metadata:
  version: 0.1.1
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-toolify` | Past corrections and preferences for this skill |
...
Use `WebFetch` or the Context7 MCP to pull the *current* official quickstart:

```bash
# Prefer the Context7 MCP if available (fresher docs than training data):
...
Offer:
- *"Set up a cron to check the webhook is still receiving events daily via `ops-cron` (or `coreyhaines-skills-loopify`)?"*
...
If a recipe doesn't exist yet, the wizard fetches the official docs, scaffolds fresh, and offers to save the recipe as `projects/coreyhaines-skills-toolify/recipes/<tool>-<stack>.md` for reuse (never inside the skill folder, updates wipe it). When looking up recipes, check both `references/` (shipped) and `projects/coreyhaines-skills-toolify/recipes/` (yours).

## Composes with

- **`coreyhaines-skills-skillify`** - sibling in `-ify` trifecta. Use it when the goal is a new SKILL.md, not an integration (AI-OS's own skill builder is `meta-skill-creator`).
- **`coreyhaines-skills-loopify`** - sibling. Use it for setting up cron/agent-loop patterns on top of the toolified integration (AI-OS's own scheduler is `ops-cron`; e.g., poll a webhook, sync data daily).
- **`mcp-builder`** - AI-OS's own skill for building a full MCP server from scratch, when wiring an existing one via `.mcp.json` isn't enough.
- **Context7 MCP** - fetch current SDK docs (fresher than training data).
- **`coreyhaines-skills-watch-video`** - if the user has a Loom of an integration walkthrough, feed it in to synthesize the recipe (a YouTube walkthrough can go through `tool-youtube`).
...
- **Recipe-first for repeat tools.** If integrating a tool the user has done before, load the recipe from `references/` or `projects/coreyhaines-skills-toolify/recipes/` and adapt, don't re-derive.
