...
**Auth:** `NOTION_API_KEY` in `.env`, or AI-OS's native Notion connector (see `docs/connectors.md`). Calling the REST API directly (below) also works.
...
**Auth:** Plane API key (`PLANE_API_KEY` in `.env`; prompt the user if missing). Workspace slug from `boards.md`.
...
4. Optionally: persist a snapshot to `projects/coreyhaines-skills-pm/boards-cache/<business>.md` so subsequent runs don't require re-pasting (with a clear warning that the cache is stale until next paste), never inside the skill folder, updates wipe it
