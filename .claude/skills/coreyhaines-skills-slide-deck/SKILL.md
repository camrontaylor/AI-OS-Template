---
name: coreyhaines-skills-slide-deck
description: "Drafts, updates, converts, or exports slide decks as TypeScript Slide[] arrays for a React/Next.js slide system, using your primitives, brand gradients, and speaker notes; modes for new, update, PPTX-to-React, and Playwright export to HTML, PDF, or Vercel URL. (needs your own React/Next.js slide repo)"
when_to_use: 'Invoke when the request sounds like: "draft a deck", "deck for [topic]", "talk on [topic]", "keynote on [topic]", "convert this pptx", "export this deck"'
metadata:
  version: 0.2.0
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-slide-deck` | Past corrections and preferences for this skill |
...
The `<AUTHOR_HANDLE>` / `<AUTHOR_SITE>` tokens (close-slide footer) come from `projects/coreyhaines-skills-slide-deck/identity.yaml` (keys: `handle`, `site`) if it exists. Otherwise ask the user for their handle and site once, offer to save them there, and use those. If they don't want a footer, drop the `<p>` entirely, never ship a deck with someone else's identity on it.
...
The archive lives in `projects/coreyhaines-skills-slide-deck/INDEX.md` (create the folder if missing). This is the AI-OS home for the skill's reusable artifacts, so it survives skill updates. Never write the archive inside the skill's own folder, updates re-sync it from source and wipe anything saved there. **Migration:** if this skill's folder contains an old `references/decks-archive.md` with user entries, move it to `projects/coreyhaines-skills-slide-deck/INDEX.md` first.

Append a one-liner to `projects/coreyhaines-skills-slide-deck/INDEX.md` (create if missing):
...
6. **Preserve the original PPTX**, never modify in place. Save under `projects/coreyhaines-skills-slide-deck/conversions/<slug>-from-pptx/` with the original file + extracted JSON for audit.
...
3. **Run Playwright snapshot**:
   ...
   - Saves PNGs to `projects/coreyhaines-skills-slide-deck/exports/<slug>-<YYYY-MM-DD>/slide-<n>.png`
4. **Combine** per output type:
   ...
   - `vercel` → `vercel deploy projects/coreyhaines-skills-slide-deck/exports/<slug>-<date>/`
5. **Report** path / URL.
...
- `coreyhaines-skills-business-brainstorm` - if a pitch deck, brainstorm the offer first; deck draws from the brief
- `coreyhaines-skills-decide` - for talks that hinge on a decision (e.g., "should I take VC money?"), decide first
- `coreyhaines-skills-deep-research` - for talks needing data the deck doesn't have yet
- `coreyhaines-skills-second-brain` - pull relevant `[[wiki]]` pages as content sources; the deck can cite them
- `coreyhaines-skills-watch-video` - turn a podcast episode, Loom, or talk recording into a deck outline (run in `visual` mode to also capture key moments + slides shown)
- `coreyhaines-skills-jab-hook` - once delivered, the talk becomes promo angles for the BIP rotation
- `frontend-skills` (external, needs your own setup) - for non-branded HTML decks where your React deck repo isn't the home
