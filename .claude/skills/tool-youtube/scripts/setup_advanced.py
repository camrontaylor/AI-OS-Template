...
Modes:
  setup_advanced.py --check      Silent preflight. Exit 0 if ready, 2/3/4 on failure.
  setup_advanced.py --json       Machine-readable status for Claude to parse.
  setup_advanced.py              Installer. Auto-installs deps, scaffolds .env, marks SETUP_COMPLETE.
...
# pip user-install locations for yt-dlp (when brew is not available)
_YTDLP_USER_PATHS = [
...
#
# Whisper transcription backend. This is the trusted transcript path when
# captions are inaccurate or when you point /watch at a local file.
#
...
#
# YouTube Data API - used only for Channel Mode (@handle / latest uploads).
# Get a key: https://console.cloud.google.com/apis/credentials
#
# Leave both blank only if local mlx-whisper is available, or if you are okay
# with frame-only output when Whisper cannot run.
...
def _have_imageio_ffmpeg() -> bool:
    """True if imageio-ffmpeg is installed (bundles its own aarch64 ffmpeg binary)."""
    try:
...
def _have_ytdlp_pip() -> bool:
    """True if yt-dlp is installed via pip at a user-install location."""
    return any(p.exists() for p in _YTDLP_USER_PATHS)


def _have_mlx_whisper() -> bool:
    """True if mlx-whisper is installed (Apple Silicon local transcription, no API key)."""
    if platform.machine() != "arm64":
        return False

    # Importing mlx can abort the interpreter when Metal is unavailable in the
    # current process. Probe in a child process so preflight stays recoverable.
    try:
...
def _check_binaries() -> list[str]:
    ...
    for b in REQUIRED_BINARIES:
        if b in ("ffmpeg", "ffprobe"):
            # imageio-ffmpeg ships a bundled ffmpeg binary. The watch pipeline
            # can also derive metadata from ffmpeg when ffprobe is unavailable.
            if not _which(b) and not _have_imageio_ffmpeg():
                missing.append(b)
        elif b == "yt-dlp":
            if not _which(b) and not _have_ytdlp_pip():
                missing.append(b)
        else:
            if not _which(b):
                missing.append(b)
...
def _have_api_key() -> tuple[bool, str | None]:
    ...
    if _read_env_key("OPENAI_API_KEY"):
        return True, "openai"
    # mlx-whisper is a local Apple Silicon backend that requires no API key
    if _have_mlx_whisper():
        return True, "mlx (local)"
    return False, None
...
def _scaffold_env() -> bool:
    ...
    if CONFIG_FILE.exists():
        ...
        additions = []
        for key in ("GROQ_API_KEY", "OPENAI_API_KEY", "YOUTUBE_API_KEY"):
            if not any(line.strip().startswith(f"{key}=") for line in existing.splitlines()):
                additions.append(f"{key}=")
        if additions:
            prefix = "" if existing.endswith("\n") else "\n"
            CONFIG_FILE.write_text(existing + prefix + "\n".join(additions) + "\n", encoding="utf-8")
            try:
...
def cmd_check() -> int:
    ...
    if not s["has_api_key"]:
        parts.append("no Whisper backend (mlx local, GROQ_API_KEY, or OPENAI_API_KEY)")
