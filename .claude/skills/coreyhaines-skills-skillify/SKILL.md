---
name: coreyhaines-skills-skillify
description: "Creates, adapts, or updates a Claude Code skill in your own sibling repos: builds one from a chat, video, dump, or scratch; ports an external skill with keep/adapt/add classification, license check, and attribution; or improves an existing skill from learnings with semver discipline. Not for integrations (coreyhaines-skills-toolify) or loops (coreyhaines-skills-loopify). (needs your own skill repos)"
when_to_use: 'Invoke when the request sounds like: "create a skill", "make this a skill", "adapt this skill", "port this skill", "update X skill", "propagate this learning"'
metadata:
  version: 0.2.0
---
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-skillify` | Past corrections and preferences for this skill |
...
| **from-video** | User recorded a Loom / Zoom / screen-share. Calls `coreyhaines-skills-watch-video` in visual mode → transcript + key visual moments → SKILL.md. Best for visual/UI-heavy workflows. |
...
- **`meta-skill-creator`** - AI-OS's own skill for creating, editing, and evaluating skills. The format authority in this workspace.
- **AI-OS `docs/skill-building.md`** - the house rules for SKILL.md structure, frontmatter, the description format, and folder naming. Read it before building.
...
Call those directly when in doubt about format. `coreyhaines-skills-skillify` orchestrates *your* workflow - which repo, which conventions, which cross-references. Format expertise lives upstream.

### Step 3 - Pick target repo

Load your sibling repo list from `projects/coreyhaines-skills-skillify/repos.yaml` if present. Otherwise default to AI-OS's own skill home, `.claude/skills/` in this workspace (where `meta-skill-creator` also builds). The repos.yaml is for your own external skill repos (needs your own skill repos). Example format:

```yaml
# projects/coreyhaines-skills-skillify/repos.yaml
repos:
  - name: aios
    domain: AI-OS's own skills
    path: .claude/skills
  - name: marketingskills
    domain: Generic marketing tactics (your own repo)
...
2. Call `coreyhaines-skills-watch-video <url> visual` to get transcript + key visual moments + summary. (For a YouTube URL, AI-OS's `tool-youtube` is the native path.)
...
7. Optionally capture the source via `str-resources` (or `coreyhaines-skills-second-brain` as `call-<slug>.md` / `note-<slug>.md` if you keep an Obsidian vault).

Heuristic: recording >10 minutes = process probably too big for one skill. Suggest splitting before drafting.

**from-scratch:**
Interactive Q&A:
1. Name (kebab-case, verb-noun preferred - matches `coreyhaines-skills-watch-video`, `coreyhaines-skills-read-book`, `coreyhaines-skills-paste`)
2. One-line purpose
...
For deeper description-writing guidance, consult `meta-skill-creator` and AI-OS `docs/skill-building.md`.
...
Body follows existing skills' pattern (`coreyhaines-skills-pm`, `coreyhaines-skills-decide`, `coreyhaines-skills-second-brain`):
...
- *"Run through `meta-skill-creator` for a quality pass?"*
...
| **Adapt** | Tool defaults, paths, voice, naming, opinions | Their video tool → your `coreyhaines-skills-watch-video`, their kanban → your `coreyhaines-skills-pm`, their voice → your voice |
| **Add** | Cross-references to your existing skills, composition notes, your conventions | "Composes with `coreyhaines-skills-decide`," "Captures via `str-resources` (or `coreyhaines-skills-second-brain` raw/)," "Uses MLX-Whisper local" |
...
**Adapt:**
- Naming: rename to verb-noun if not (matches `coreyhaines-skills-watch-video`, `coreyhaines-skills-read-book`, `coreyhaines-skills-paste`)
- Tool swaps: their generic kanban → `coreyhaines-skills-pm`, their video tool → `coreyhaines-skills-watch-video` (or `tool-youtube` for YouTube), their note system → `coreyhaines-skills-second-brain` or `str-resources`, their decision framework → `coreyhaines-skills-decide`
- Voice: apply your voice rules - direct, conviction-coded. For social-adjacent skills, apply link-placement rule.
- Paths: their `~/outputs/` → your `projects/<skill-folder>/` convention; their config → your `references/` pattern
...
- Offer:
  ...
  - *"Run a QA pass through `meta-skill-creator`?"*
  - *"Set up an upstream-check reminder via `ops-cron` (or `coreyhaines-skills-loopify`)?"*
...
```bash
# AI-OS's own skills live here:
grep -rln "<key terms>" .claude/skills/ --include="*.md"

# Across your external sibling repos (list yours in projects/coreyhaines-skills-skillify/repos.yaml):
config="projects/coreyhaines-skills-skillify/repos.yaml"
for raw in $(yq -r '.repos[].path' "$config"); do
  repo="${raw/#\~/$HOME}"  # expand leading ~ to $HOME so grep resolves the real path
  grep -rln "<terms>" "$repo" --include="*.md" 2>/dev/null
done
```
...
| Cross-cutting principle | `context/learnings.md` (under `# General`), or a durable fact to `context/MEMORY.md` via `meta-memory-write`. Never write to native Claude memory (`~/.claude/memory/`) - AI-OS keeps it off. |
| Both | Record the learning AND update the skill(s) that immediately apply |

Example: *"links go in first comments, not body"* → applies to `coreyhaines-skills-jab-hook` AND is a broader social principle → both update jab-hook AND record the lesson in `context/learnings.md` under `# General`.

Offer the write explicitly: *"This looks like a principle, not just a skill rule. Save it to `context/learnings.md` (or `context/MEMORY.md` via `meta-memory-write`)?"*
...
- Multi-skill propagation: `"jab-hook, coreyhaines-marketing-social: link placement (no inline URLs)"`
...
- Whether `meta-skill-creator` would be a useful QA pass
...
- **`coreyhaines-skills-watch-video`** - load-bearing for CREATE / from-video. Visual-mode output (transcript + key visual moments + summary) is the input for skill synthesis. Always call in `visual` mode for process recordings, UI state matters as much as words.
- **`str-resources`** (or `coreyhaines-skills-second-brain`) - optionally capture the source video/dump as a durable AI-OS resource so the source artifact lives alongside the skill it produced.
- **`coreyhaines-skills-toolify`** - sibling in the `-ify` trifecta. Use it when the goal is adding an integration/MCP/API, not authoring a skill (AI-OS's own MCP builder is `mcp-builder`).
- **`coreyhaines-skills-loopify`** - sibling in the `-ify` trifecta. Use it for agent-loop setup rather than a skill (AI-OS's own scheduler is `ops-cron`).
- **`meta-skill-creator`** - AI-OS's own skill for creating, editing, and evaluating skills. Call it for format and best-practices expertise, and for a QA pass after any mode.
...
- **Verb-noun naming** where possible (matches `coreyhaines-skills-watch-video`, `coreyhaines-skills-read-book`, `coreyhaines-skills-paste`). Single-word noun names are fine for distinctive concepts (`coreyhaines-skills-decide`, `coreyhaines-skills-pm`, `coreyhaines-skills-skillify`).
