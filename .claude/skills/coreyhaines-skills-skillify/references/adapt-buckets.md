...
|---|---|---|
| **Naming** | Their convention | Verb-noun (matches `coreyhaines-skills-watch-video`, `coreyhaines-skills-read-book`, `coreyhaines-skills-skillify`) when possible |
| **Tool defaults** | Their video tool | Your `coreyhaines-skills-watch-video` (or `tool-youtube` for YouTube) |
| | Their kanban | Your `coreyhaines-skills-pm` |
| | Their transcript tool | Your `coreyhaines-skills-watch-video transcript` mode |
| | Their notes / KB | Your `coreyhaines-skills-second-brain`, or `str-resources` for durable AI-OS resources |
| | Their decision framework | Your `coreyhaines-skills-decide` |
| | Their social drafting | Your `coreyhaines-skills-jab-hook` |
| | Their research | Your `coreyhaines-skills-deep-research` |
| **Paths** | `~/outputs/` or `~/notes/` | `projects/<skill-folder>/` convention |
...
Examples:
- "Composes with `coreyhaines-skills-decide`" (when the skill hits a decision moment)
- "Captures via `str-resources` (or `coreyhaines-skills-second-brain` raw/ as `<prefix>-`)"
- "Falls back to `coreyhaines-skills-social-fetch` for the URL fetch step"
- "Uses MLX-Whisper local (see install in `coreyhaines-skills-watch-video` references)"
- Learning references: cross-cutting lessons in `context/learnings.md` (never native Claude memory)
