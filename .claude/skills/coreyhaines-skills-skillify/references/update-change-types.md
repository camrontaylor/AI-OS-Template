...
- Spin off as a new skill via this skill's CREATE mode (or `meta-skill-creator`)
...
**Examples from this session:**
- Installed `uv` + MLX-Whisper → `coreyhaines-skills-watch-video` references this in transcript backend selection
- ScrapeCreators / Apify keys → `coreyhaines-skills-social-fetch` paid-fallback strategies
