...
The unique value of `coreyhaines-skills-skillify`'s UPDATE mode. A learning often applies to multiple skills - surface all of them so the user can approve per file.
...
|---|---|
| Link placement rule (no URLs in body) | `coreyhaines-skills-jab-hook` + a cross-cutting lesson in `context/learnings.md` (applies to any other social skills in your sibling repos too) |
| Verb-noun naming convention | `coreyhaines-skills-watch-video`, `coreyhaines-skills-read-book` (rename pass across multiple skills) |
| Anti-AI-slop banned phrases | `coreyhaines-skills-slide-deck`, `coreyhaines-skills-jab-hook` (both write content; both should ban "dive deep," "leverage," etc.) |
| Spoken-aloud voice rule | `coreyhaines-skills-slide-deck` (load-bearing); `coreyhaines-skills-jab-hook` partial relevance (social IS read silently usually, but voice still applies) |
...
```bash
# Example: link placement learning. AI-OS's own skills live in .claude/skills/
grep -rln "URL\|link\|outbound\|first comment" .claude/skills/ --include="*.md"
...
for raw in $(yq -r '.repos[].path' "$config" 2>/dev/null); do
  ...
  if [ -d "$repo" ]; then
    grep -rln "URL\|link\|outbound\|first comment" "$repo" --include="*.md" 2>/dev/null
  fi
done
```
...
Example: a "voice should be spoken aloud" rule won't grep-match in `coreyhaines-skills-pm` (project management skill - no voice content), but it applies in any skill that produces user-facing text (`coreyhaines-skills-slide-deck`, `coreyhaines-skills-jab-hook`, `coreyhaines-skills-read-book` summaries, a newly authored skill's output).
...
### Learning candidate (cross-cutting principle?)
- Yes / no. If yes, record it in `context/learnings.md` under `# General` (or `context/MEMORY.md` via `meta-memory-write` for a durable fact). Never native Claude memory.
...
- The skill's context is genuinely different (e.g., `coreyhaines-skills-read-book` "summary" mode wants written voice, not spoken)
- The rule has a documented exception (e.g., reading-first density mode in `coreyhaines-skills-slide-deck` exempts voice rules)
...
If the same learning lands in update proposals 3+ times for related skills, it's probably a **principle, not a rule**. Promote it to `context/learnings.md` under `# General` and reference from each affected skill rather than copy-pasting the same text 5 places.
