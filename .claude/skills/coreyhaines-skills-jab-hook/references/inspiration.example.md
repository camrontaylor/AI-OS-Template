...
**Copy this file to `projects/coreyhaines-skills-jab-hook/inspiration.local.md` and replace with your real list.** The local copy is gitignored.

## Setup

```bash
mkdir -p projects/coreyhaines-skills-jab-hook
cp .claude/skills/coreyhaines-skills-jab-hook/references/inspiration.example.md \
   projects/coreyhaines-skills-jab-hook/inspiration.local.md
# Then edit projects/coreyhaines-skills-jab-hook/inspiration.local.md with your real accounts
```

## Schema - organize by platform

What works on LinkedIn doesn't work on X (and vice versa). Organize the file with one section per platform you draft for. The `coreyhaines-skills-jab-hook` skill scans accounts from the section matching the draft's target platform.
...
## YouTube
Long-form video. (More relevant to tool-youtube than coreyhaines-skills-jab-hook.)
...
## How `coreyhaines-skills-jab-hook` uses your list
...
3. Pulls recent posts via `coreyhaines-skills-social-fetch` (uses `agent-browser` for LinkedIn if no paid keys; ScrapeCreators for full X threads when `SCRAPECREATORS_API_KEY` is set)
...
Extracted patterns accumulate in `projects/coreyhaines-skills-jab-hook/patterns.local.md`, tagged with `platform: <linkedin | x | …>` so the skill knows which patterns transfer where.
