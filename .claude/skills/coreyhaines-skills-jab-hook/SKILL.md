---
name: coreyhaines-skills-jab-hook
description: "Plans an X and LinkedIn posting rotation using Gary Vaynerchuk's jab-jab-jab-right-hook (mostly value posts so promo asks land), drafting into Typefully; modes for a 7-day plan, next post, coverage audit, or a specific draft. (needs your own Typefully workspace and properties.yaml setup)"
when_to_use: 'Invoke when the request sounds like: "what should I post", "plan my socials", "next promo", "next jab", "social rotation", "audit my socials"'
metadata:
  version: 0.3.1
---
...
**One-time setup**: see `references/properties.md` and `references/typefully-config.md` to configure your real portfolio + workspace. Personal config lives in `projects/coreyhaines-skills-jab-hook/`.
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-jab-hook` | Past corrections and preferences for this skill |

## Mental model

- **N rotation slots** (configured in `projects/coreyhaines-skills-jab-hook/properties.yaml`): one per property you want to promote on rotation
...
1. Read `references/properties.md` - the configured rotation slots and angle ideas
...
4. Pull recent posts from your personal Typefully workspace:
   - First run: call `mcp__typefully__typefully_list_social_sets` and ask which social set is your personal (X + LinkedIn). Save the ID to `projects/coreyhaines-skills-jab-hook/typefully.yaml` for future runs (schema in `references/typefully-config.example.yaml` - never save it inside the skill folder; upgrades wipe it).
   - Call `mcp__typefully__typefully_list_drafts` filtered to the last 30 days
   - Classify each as promo / BIP / educational by content
5. Compute **days since last promo** for each configured slot
...
5. Log new patterns in `projects/coreyhaines-skills-jab-hook/patterns.local.md` (create if missing) so they accumulate - `references/patterns.md` documents the starter patterns and stays read-only
...
If yes:
1. Read social set ID from `projects/coreyhaines-skills-jab-hook/typefully.yaml` (setup notes in `references/typefully-config.md`)
...
- `coreyhaines-skills-paste` - clean output for pasting into Typefully / LinkedIn / X manually if the MCP path doesn't fit
- `coreyhaines-skills-social-fetch` - pull inspiration-account posts for structural analysis (replaces inline agent-browser)
- `coreyhaines-skills-second-brain` - the `Content Ideas` wiki page hoppers hooks, frameworks, stories; `coreyhaines-skills-jab-hook` drafts pull candidates from there
- `coreyhaines-skills-deep-research` - when a promo needs a stat or citation the draft doesn't have yet
- `coreyhaines-marketing-social` (or AI-OS's `mkt-content-repurposing`) - generic social frameworks, useful when teaching strategy vs shipping your own posts
- `coreyhaines-marketing-copywriting` (or AI-OS's `mkt-copywriting`) - for hook / headline ideation when stuck
...
- **Links belong in first comments, not the body** for LinkedIn + X. Documented in `context/learnings.md` under `## coreyhaines-skills-jab-hook`. Body posts with inline URLs get algorithm-suppressed on both platforms.
...
- **Voice.local.md overrides the shipped voice.md.** Personal voice rules never live in the public repo - always in `projects/coreyhaines-skills-jab-hook/voice.local.md`.
- **Cadence cap: 2 posts/day per platform, per account.** More than that degrades reach + reads as spam.

## Memory references

Voice and cadence lessons live in AI-OS learnings, not native Claude memory (which AI-OS keeps off). Read `context/learnings.md` under `## coreyhaines-skills-jab-hook` for:

- conviction-coded CTAs + reader-perspective framing
- cadence cap (e.g., 2 posts/day per platform)
