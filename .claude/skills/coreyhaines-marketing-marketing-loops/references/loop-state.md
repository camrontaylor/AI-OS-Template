...
Persist each loop's state in a file under `projects/coreyhaines-marketing-marketing-loops/loops/` - the AI-OS convention for a skill's own reusable artifacts (per-skill work lives in `projects/<skill-folder-name>/`). One state file per loop:

```
projects/coreyhaines-marketing-marketing-loops/loops/<loop-name>.json     # the loop's memory
projects/coreyhaines-marketing-marketing-loops/loops/<loop-name>.log      # append-only run log
```
