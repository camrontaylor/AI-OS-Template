...
3. Add to `.env` (gitignored; also add the name to `.env.example`):
   ```bash
   SCRAPECREATORS_API_KEY=<your-key>
   ```
4. Reload your shell / start a new session
...
AI-OS can also reach Apify through Composio (see `docs/connectors.md`) without a local key, per the Web Data routing in AGENTS.md.
...
- Always check the cache first (`projects/coreyhaines-skills-social-fetch/_cache/` if it exists)
...
- You're starting on a skill where social capture is central
