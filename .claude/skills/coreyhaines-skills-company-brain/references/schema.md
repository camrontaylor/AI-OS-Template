...
Projects/              → Team task management (DO NOT modify - coreyhaines-skills-pm owns it)
...
Same as `coreyhaines-skills-second-brain`:
...
## Publishing (same as coreyhaines-skills-second-brain)

Use pandoc + a shared stylesheet at `projects/coreyhaines-skills-company-brain/render.css`:

```bash
pandoc "outputs/<file>.md" \
  --css projects/coreyhaines-skills-company-brain/render.css \
  --metadata title="<title>" \
  --pdf-engine=xelatex \
  -o "outputs/<file>.pdf"
```

Company brain outputs commonly leave the vault as: client-facing briefs, quarterly reviews, sales enablement PDFs, onboarding docs. Same rendering pattern as coreyhaines-skills-second-brain.
