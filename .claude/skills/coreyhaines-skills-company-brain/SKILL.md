---
name: coreyhaines-skills-company-brain
description: "Build and query a shared, AI-ready team knowledge base - people, companies, meetings, SOPs, and decisions - with multi-author trust levels and sensitivity tags. Seven modes: capture, compile, query, review, lint, connect, search (needs your own vault setup). Not for a personal knowledge base (see coreyhaines-skills-second-brain)."
when_to_use: 'Invoke when the request sounds like: "capture this into the team brain", "log this meeting", "what does the team know about X", "compile the company wiki"'
metadata:
  version: 0.2.0
---
...
Team-scope sibling to `coreyhaines-skills-second-brain` (personal-scope). Same core compile → wiki → outputs pattern; different raw schema optimized for multi-author, sales-heavy, ops-heavy team use.
...
| File | Load level | Why |
|---|---|---|
| `context/learnings.md` | `## coreyhaines-skills-company-brain` | Past corrections and preferences for this skill |

## Mental model

Three layers, same as coreyhaines-skills-second-brain - but the raw/ layer is *structured*, not flat:
...
| `decisions/` | Decision records (narrative form; `coreyhaines-skills-decide` skill's structured form is different). |
...
`wiki/`, `outputs/`, and `INDEX.md` work the same as coreyhaines-skills-second-brain.
...
1. Read `references/vault-config.md` for the vault path (default: `${COMPANY_BRAIN_VAULT:-projects/coreyhaines-skills-company-brain}/`). Set `COMPANY_BRAIN_VAULT` if you keep the team brain in an external Obsidian or Git-backed vault instead.
...
**Same intake mechanics as coreyhaines-skills-second-brain, but the routing is different - pick the structured dir based on content type.**
...
Same core pattern as `coreyhaines-skills-second-brain`'s compile mode - process unprocessed structured-raw files into wiki pages, update INDEX.md, add Sources sections.

**Differences from coreyhaines-skills-second-brain:**
...
Same as coreyhaines-skills-second-brain query, plus:
...
- **Route external gaps to `coreyhaines-skills-deep-research`**, same as coreyhaines-skills-second-brain.
...
**Cadence**: weekly for active vaults; pair with `coreyhaines-skills-loopify` to schedule it so the cull actually happens instead of depending on someone remembering. A vault where reviews lapse >1 month shows up in lint (check 13).

### lint

Same seven checks as coreyhaines-skills-second-brain PLUS:
...
Same as coreyhaines-skills-second-brain plus **cross-category link suggestions** - e.g., `sales-objections/pricing-too-high.md` should link to `customer-language/willingness-to-pay.md` and `sops/discovery-call-cadence.md` if they exist.
...
Auto-sync is optional - most teams start with manual capture and add automation as the vault matures. Pair with `coreyhaines-skills-loopify` to schedule periodic sync jobs.

## Composes with

- **`coreyhaines-skills-second-brain`** - sibling. Use `coreyhaines-skills-second-brain` for your personal wiki; `coreyhaines-skills-company-brain` for the team's. A person can maintain both simultaneously with separate vault paths.
- **`coreyhaines-skills-skillify`** - use to author new skills that read from the company brain (e.g., a `weekly-team-brief` skill that queries `coreyhaines-skills-company-brain` every Monday).
- **`coreyhaines-skills-loopify`** (or AI-OS's own `ops-cron`) - schedule auto-sync jobs (Fathom pull daily, Slack export weekly, review pass weekly, INDEX lint monthly).
- **`coreyhaines-skills-toolify`** - wire up integrations that feed the company brain (Fathom webhook receiver, Attio API, etc.).
- **`coreyhaines-skills-deep-research`** - when `query` finds gaps, route external. Save coreyhaines-skills-deep-research results into `raw/` for future compilation.
- **`coreyhaines-skills-decide`** - `decisions/` folder complements `coreyhaines-skills-decide`'s structured archive. `coreyhaines-skills-decide` records the *evaluation*; `decisions/` records the *narrative + outcome + review notes*.
- **`coreyhaines-skills-pm`** - team task management sits in `Projects/` (reserved from company-brain). `coreyhaines-skills-pm` owns Projects/; company-brain reads it for context but doesn't modify.
- **`coreyhaines-skills-jab-hook`** - `customer-language/` fuels social copy that resonates with actual prospect language.
...
Same lineage as `coreyhaines-skills-second-brain`:
...
- **Never delete raw files.** Same rule as coreyhaines-skills-second-brain - the structured dirs are the source of truth. When info is wrong or stale, **deprecate, don't delete** - `trust: deprecated` removes it from context while preserving history.
