...
Reuses tooling from the `coreyhaines-marketing-social` skill's listening workflow. Same install: `brew install jq`.
...
For repeatable monitoring, add a "Newsjacking topics" section to `projects/coreyhaines-marketing-social/listening-sources.md` (template in the `coreyhaines-marketing-social` skill's references):
