...
| File | Load level | Why |
|---|---|---|
| `.claude/skills/*/SKILL.local.md` | every file found | Candidate local overrides |
| Matching `SKILL.md` | full | Base used for the comparison |
| `context/learnings.md` | `## meta-synthesize-locals` | Known synthesis exceptions |
...
Run this manual eval before changing synthesis behavior:

1. Create a temporary skill with a `SKILL.local.md` that is an exact copy of
   `SKILL.md` plus one dated local rule. Pass if synthesis keeps only the local
   rule.
2. Create a lean `SKILL.local.md` containing only `## Rules`. Pass if synthesis
   leaves it unchanged.
3. Create a local file with a genuinely modified section. Pass if synthesis
   keeps that section and removes only duplicated base content.

The eval fails if local-only rules are lost, base content remains duplicated, or
an already-lean local override is rewritten unnecessarily.
