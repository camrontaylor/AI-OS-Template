---
name: viz-make-interfaces-feel-better
description: "Review or apply interface-polish details in frontend code - concentric border radius, optical alignment, shadows vs borders, animations, tabular numbers, text wrapping, image outlines, scale-on-press, hit areas, and icon stroke weight. Returns prioritized findings with file:line, before/after, and a Block/Needs-changes/Approve verdict in quick or full mode. Not for broad UX/accessibility audits (see impeccable), building features (see eng-implement), or ad creative (see viz-ad-creative-*)."
when_to_use: 'Invoke when the request sounds like: "make it feel better", "this feels off", "polish this UI", "review my component", "the animation feels janky", "border radius looks wrong", "icons look off-center", or any micro-interaction / visual-detail pass on built UI.'
---
...
| Need | Load level | Why |
|---|---|---|
| The target UI source files (or the live screen/component) | full | The review cites `file:line`; you must read the actual implementation, not describe from memory |
| Project styling system (`package.json`, `tailwind.config`, existing CSS/CSS-in-JS) | targeted | Express every fix in the project's own system; never introduce a second one |
| `motion` / `framer-motion` presence in `package.json` | targeted | Decides icon-animation recipe (spring config vs CSS cross-fade) |
| `references` beside this file (`typography.md`, `surfaces.md`, `animations.md`, `icons.md`, `performance.md`) | per-category | Detailed recipes; load the category the finding touches |
| `context/learnings.md` -> `## viz-make-interfaces-feel-better` | targeted | Prior polish-review lessons for this workspace |

This skill inspects and reviews only; it does not need brand context or memory recall. No MCP, `.env` key, or external service is required.
