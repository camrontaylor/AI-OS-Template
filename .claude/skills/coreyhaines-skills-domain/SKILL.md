---
name: coreyhaines-skills-domain
description: "Brainstorms and checks available .com domains for a new brand: name ideas, availability, aftermarket pricing, USPTO trademark screening, and social-handle checks. Not for DNS setup or deploying to a domain you already own."
when_to_use: 'Invoke when the request sounds like: "find a domain", "check domain availability", "what .com is available for X", "name my project", "is X.com available", "trademark check for X"'
---
...
| File | Load level | Why |
|---|---|---|
| Active scope `brand_context/positioning.md` | full, when present | Naming angle and category |
| Active scope `brand_context/icp.md` | summary, when present | Buyer language and trust constraints |
| Active scope `context/learnings.md` | `## tool-domain` | Prior naming and availability lessons |
...
- `agent-browser` skill with real Chromium → Cloudflare fingerprints Playwright automation flags, serves challenge pages. Bypassing needs `playwright-extra` + stealth plugin (flaky) or pre-warmed browser profile with human-solved captcha (not worth it for a domain hunt)
...
- **`.env` keys** - add the optional `DOMAINR_API_KEY` and `NAMECHEAP_API_*` keys (Step 0) to unlock the aftermarket + price cross-checks; the workflow runs without them.
- **`q-question`** - pressure-test the underlying business idea BEFORE the hunt (a bad idea with a great .com is still a bad idea), and for the "which of the top 3 candidates" call.
- **`agent-browser`** - drives the USPTO trademark SPA in Step 10a (installed globally as an AI-OS skill).
- **`meta-skill-creator`** - if the hunt surfaces a repeatable niche-naming pattern worth capturing, scaffold it as its own skill.
