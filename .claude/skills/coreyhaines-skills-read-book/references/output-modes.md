...
Styling is optional. Set `READBOOK_CSS` to a stylesheet path (system-font + max-width ~720px + print-optimized works well) to brand the output; unset, pandoc uses its clean defaults.
...
- Don't auto-render unless `--render` is specified - most book notes stay as markdown for the workspace
...
Tradeoff: gains rich publishing features; costs an extra tool and adds frontmatter complexity. Skipped by default.
