...
make_fixture() {
  rm -rf "$TEST_ROOT"
  mkdir -p "$TEST_ROOT/repo/scripts" "$TEST_ROOT/repo/projects/system-health" "$TEST_ROOT/repo/.command-centre"
...
if ! (cd "$TEST_ROOT/repo" && bash scripts/memsearch-health.sh > "$TEST_ROOT/ok.out"); then
...
if (cd "$TEST_ROOT/repo" && bash scripts/memsearch-health.sh > "$TEST_ROOT/degraded.out"); then
...
EOF
chmod +x "$TEST_ROOT/repo/scripts/memsearch-search.sh"
if (cd "$TEST_ROOT/repo" && bash scripts/memsearch-health.sh > "$TEST_ROOT/attention.out"); then
